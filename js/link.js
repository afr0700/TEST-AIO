        ...
        /**
         * Mengambil daftar URL dari GitHub dan mengubahnya menjadi array bugOptions.
         */
        async function getBugOptions() {
            const dataUrl = "customdomain.txt?v=" + new Date().getTime();
            let urlList = [];
            ...
            try {
                ...
                // Filter baris kosong
                urlList = textData.split('\n').filter(line => line.trim() !== '');
            } catch (error) {
                console.error("Gagal memuat daftar URL bug:", error);
                // Biarkan urlList kosong, tapi 'Default' dan 'Manual' akan tetap ada
            }

            // Kembalikan array bugOptions yang sudah jadi
            return [
                { value: "", label: "Default" },
        ...
        /**
         * Mengisi semua dropdown bug (vless, trojan, ss) dengan data.
         */
        async function populateAllBugDropdowns() {
            // ID dari semua dropdown yang ingin diisi
            const dropdownIds = ["vless-bug", "trojan-bug", "ss-bug"];
            
            // Tampilkan "Memuat..." di semua dropdown
            dropdownIds.forEach(id => {
                const dropdown = document.getElementById(id);
                if (dropdown) {
                    dropdown.innerHTML = "<option value=''>Memuat bug...</option>";
                }
            });

            // 1. Ambil data (hanya sekali)
            const bugOptionsData = await getBugOptions();
            
            // Simpan ke variabel global jika script1.js perlu membacanya
            bugOptions = bugOptionsData; 

            // 2. Loop setiap ID dropdown dan isi
            dropdownIds.forEach(id => {
                const dropdown = document.getElementById(id);
                if (!dropdown) {
                    // Jika elemen tidak ditemukan, lewati
                    return; 
                }
                
                // Kosongkan dari "Memuat..."
                dropdown.innerHTML = ""; 

                // 3. Loop setiap item data dan buat <option>
                bugOptionsData.forEach(item => {
            ...
            // console.log("Semua dropdown bug berhasil dimuat."); // Opsional: untuk debugging
        }

        // --- PANGGIL FUNGSI UNTUK MEMULAI ---
        // Panggil fungsi ini saat script dimuat
        // Ini akan berjalan, mengambil data, dan mengisi dropdown
        // sebelum atau saat script1.js (defer) dieksekusi.
        populateAllBugDropdowns();
