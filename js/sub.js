        /**
         * JavaScript for the subscription page (Updated for Random Domain Selection)
         */
        ...
        async function copyToClipboard(text) {
            try {
                // Menggunakan document.execCommand('copy') sebagai fallback yang lebih aman di lingkungan iFrame
        ...
        document.addEventListener("DOMContentLoaded", () => {
            // Populate main domains dropdown (Sekarang dengan pilihan acak)
            populateMainDomains()
        ...
        // Populate main domains dropdown and select a random one by default
        function populateMainDomains() {
            mainDomainSelect.innerHTML = "" // Clear existing options
            ...
            // LOGIKA BARU: Pilih domain secara acak saat dimuat
            if (CONFIG.mainDomains.length > 0) {
                const randomIndex = Math.floor(Math.random() * CONFIG.mainDomains.length)
                // Setel nilai elemen select ke nilai domain yang dipilih secara acak
                mainDomainSelect.value = CONFIG.mainDomains[randomIndex]
            }
        }
...
`
    ...
    "route": {
        ...
        async function checkProxyStatus(proxy) {
            try {
                // Gunakan backoff eksponensial untuk mengatasi throttling
                const maxRetries = 3;
                let delay = 1000;
                ...
                for (let i = 0; i < maxRetries; i++) {
                    ...
                    const timeout = setTimeout(() => controller.abort(), 5000); // Batas waktu 5 detik

                    try {
                        const response = await fetch(`${CONFIG.apiCheckUrl}${proxy.ip}:${proxy.port}`, { signal });
                        ...
                        if (error.name === 'AbortError' || i === maxRetries - 1) {
                            // Ini adalah upaya terakhir atau timeout, anggap saja gagal
                            return false;
                        }
                        // Gagal, coba lagi setelah jeda
                        await new Promise(resolve => setTimeout(resolve, delay));
                        delay *= 2; // Eksponensial backoff
                    }
                }
                return false; // Jika semua percobaan gagal
            } catch (error) {
